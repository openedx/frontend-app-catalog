...
![Custom card view component in Course catalog page data table card view slot](./images/screenshot_custom_with_card.png)

The following `env.config.tsx` example demonstrates how to replace the Course catalog page data table card view slot with a custom component that uses the plugin props (`displayData`). In this case, it creates a custom card view layout that displays course cards in a different format with additional information.
