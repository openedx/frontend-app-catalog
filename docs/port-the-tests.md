...
- **`legacy/src/header/CatalogHeader.test.tsx`** (184 LOC) and **`legacy/src/header/hooks/useMenuItems.test.tsx`** (159 LOC) — the header rewrote from a component + hook (which produced `{ mainMenu, secondaryMenu }` for `@edx/frontend-component-header`) into `src/widgets/CatalogHeader/`, a widget sub-app that declares slot operations against the shell header's `primaryLinks` slot. There's no `CatalogHeader` component and no `useMenuItems` hook to port to. **Skip both.** Replaced by a single new file `src/widgets/CatalogHeader/app.test.tsx` covering the surviving behavior (which is Batch E's whole output).
...
- **Batch D — Heavy rewrites (2 files, ~2,250 LOC).** Files: `catalog/CatalogPage.test.tsx` (1695 LOC — largest single file), `course-about/CourseAboutPage.test.tsx` (558). These get their own batch because the CatalogPage suite alone is ~35% of the total LOC and will drive its own set of small mock-shape decisions. (`header/CatalogHeader.test.tsx` was originally listed here too but belongs in Batch E — same header-widget rewrite as `useMenuItems`.)
- **Batch E — Header widget test rewrite (1 new file replaces 2 legacy files).** The header rewrote from a `CatalogHeader` component + `useMenuItems` hook into a widget sub-app at `src/widgets/CatalogHeader/` that declares slot operations for individual MenuItem widgets. Neither legacy file has a straight port target — both go on the skip-list. In their place, one new file `src/widgets/CatalogHeader/app.test.tsx` covers the behavior that still lives in our code: the 4 slot-operation condition callbacks (which MenuItem appears under which auth+config combo) and a sanity render of the URL-carrying MenuItems (Courses, Programs). Role-based MenuItems (Discover, ExploreCourses) delegate URL resolution to frontend-base's `LinkMenuItem` and are covered structurally, not through a full render, because `LinkMenuItem` imports `getUrlByRouteRole` from its own internal subpath — reaching that mock would target frontend-base's internals.
...
- 35 legacy test files → 35 ports in `src/**/*.test.*` (Batches A–D). Batch E replaces 2 more legacy files with 1 new file (`src/widgets/CatalogHeader/app.test.tsx`) — the widget rewrite of the header means the legacy tests have no straight port target. Skip-list total: 7 legacy files (5 pre-planned + `utils.test.ts` root + 2 header tests). One commit per file. Extract shared helpers into `src/test-utils/` only if a duplicated pattern shows up 3+ times mid-port.
